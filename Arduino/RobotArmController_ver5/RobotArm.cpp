...
void RobotArm::Initialize_ROM() {
	...
	//初期位置に移動
	MovePosition(robotArmSettings.initPosX, robotArmSettings.initPosY, robotArmSettings.initPosZ);
...
//位置指定
void RobotArm::MovePosition(int _x, int _y, int _z) {
	//位置情報を保存
	this->x = _x;
	this->y = _y;
	this->z = _z;
...
//終点位置とそれに関わる情報指定
void RobotArm::SetPosition(int _x, int _y, int _z, int _finFrame) {
	startX = x; desX = _x;
	startY = y; desY = _y;
	startZ = z; desZ = _z;
	finFrame = _finFrame;
...
void RobotArm::ProcessOrder(char* order) {
	...
	if (strcmp(words[0], "G90") == 0) {
	...
	else if (strcmp(words[0], "M502") == 0) {
		//移動フレーム設定
		Serial.println("Command M502");
		if (words[1][0] == 'F')
			robotArmSettings.initFrame = atoi(&words[1][1]);
	}
		...
		Serial.print("Frame:");
		Serial.println(robotArmSettings.initFrame);
...
//ロボットアームの座標更新
void RobotArm::UpdatePosition() {

	if (nowFrame == finFrame) {
		MovePosition(desX, desY, desZ);
		nowFrame++;
	}
	else if (nowFrame < finFrame) {
		//移動方式で場合分け
		switch (moveType) {
		case LINEAR:
			MovePosition(
				startX + ((desX - startX)*nowFrame) / finFrame,
				startY + ((desY - startY)*nowFrame) / finFrame,
				startZ + ((desZ - startZ)*nowFrame) / finFrame);
			break;
		case S_SHAPE:
			MovePosition(
				(int)(startX + (desX - startX)*0.5*(1 - cos(PI*nowFrame / (float)finFrame))),
				(int)(startY + (desY - startY)*0.5*(1 - cos(PI*nowFrame / (float)finFrame))),
				(int)(startZ + (desZ - startZ)*0.5*(1 - cos(PI*nowFrame / (float)finFrame))));
			break;
		}
		nowFrame++;
	}
